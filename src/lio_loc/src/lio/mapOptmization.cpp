...
public:
    ...
    */
    mapOptimization()
    {
        ...
        subCloud = nh.subscribe<dlio_loc::cloud_info>("lio_sam/deskew/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        // 订阅GPS里程计
        ...
        //@yjf
        pubGlobalMap          = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/localization/global_map", 1);
        pubSLAMInfo           = nh.advertise<dlio_loc::cloud_info>("lio_sam/mapping/slam_info", 1);
    ...
    */
    ...
    void loadGlobalMap() {
        ...
        pcl::io::loadPCDFile<PointType>(savePCDDirectory + "/FullMap.pcd", *laserCloudSurfFromMap);
        if (laserCloudSurfFromMap == nullptr) {
            ROS_ERROR("no pcd");
            return;
        }
    ...
    */
    void surfOptimization()
    {
        ...
        //@yjf
        laserCloudSurfLastDS->is_dense = false;
        std::vector<int> indices;
        pcl::PointCloud<PointType>::Ptr laserCloudSurfLastDSNoNan(new pcl::PointCloud<PointType>());
        pcl::removeNaNFromPointCloud(*laserCloudSurfLastDS, *laserCloudSurfLastDSNoNan, indices);
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            // 平面点（坐标还是lidar系）
            pointOri = laserCloudSurfLastDSNoNan->points[i];
    ...
    */
    void scan2MapOptimization()
    {
        ...
        if (laserCloudSurfLastDSNum > 30)
        {
            ...
            ROS_WARN("Not enough features! Only and %d planar features available.", laserCloudSurfLastDSNum);
        }
    }
